locals {
  service_url = google_cloud_run_service.examsched.status[0].url
}
...
output "result" {
  value = <<EOF
    ...
    gcloud builds submit --config .cloudbuild/build-migrate-deploy.yaml --project ${var.project} \
      --substitutions="_REGION=${google_cloud_run_service.examsched.location},_INSTANCE_NAME=${google_sql_database_instance.postgres.name},_SERVICE=${google_cloud_run_service.examsched.name}"
